...
**2. Get the repo (2 minutes).** This repository is the pre-code baseline — clone it (or copy its contents into a fresh private repo). The layout is already what the build expects, with **nothing to copy or rearrange**:
- **`CLAUDE.md` and `PROGRESS.md` at the repo root** — the standing rules the build obeys + the phase-gate ledger.
- The full design set (all the `.md` files) and the four `.html` mockups under **`docs/`**.
- **No code yet** — Phase 0 creates the solution. If you started a fresh repo from these files, commit this baseline: `docs v1.9, pre-code baseline`.
