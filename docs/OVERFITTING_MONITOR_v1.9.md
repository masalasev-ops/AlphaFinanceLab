...
7. **v1.8 (D56): S3 thresholds become track-length-aware trajectories.** Flat percentile cuts contradicted the master doc's own power math — a genuine edge of the size this arena can adjudicate sits in the 60th–90th percentile of its noise band for years, so flat 95/80 cuts would have made permanent Warning/Suspect the steady state for every honest strategy. Phase-4 calibration now produces `P_noise(t)` / `P_edge(t)` curves and S3 judges against them (see S3 below and MASTER §20.7). The retained *pre-calibration* anchors are correspondingly Healthy ≥ 95th / **Suspect below the 25th, SUSTAINED** (v1.9.42, Change 3: a single dip is Warning, matching the trajectory — §3) — the Suspect anchor is the anti-predictive tail (D63), so a merely edgeless strategy sitting near its band's median never trips it (Appendix A).
...
⚙ **v1.8 (D56) — trajectory thresholds:** Phase-4 replay calibration produces two percentile-vs-track-length curves — `P_noise(t)`, the envelope below which a planted **no-edge** strategy falls at the configured false-alarm rate, and `P_edge(t)`, the **median trajectory of a planted 2%/yr edge** — all plants per **D64** (MASTER §20.9): regime-conditional, autocorrelated, ≥50 seeds each, with the plant-sensitivity check archived. At track length t: **Suspect** below `P_noise(t)` sustained (anti-predictive detection stays fast at every horizon) · **Healthy** above `P_edge(t)` sustained · **Warning** between — and, since D148, **Warning also when the path is ABOVE `P_edge(t)` but the streak is not yet met**, which "between" does not describe. The band is therefore four outcomes, not three. **Contribution tokens** (the `status contribution` §4 names but never enumerated): `below_noise`, `above_edge`, `approaching_edge` (above the curve, not yet sustained), `healthy_anchor` (the pre-calibration flat-anchor form). The flat anchors (Healthy ≥ 95 / Suspect below the 25th — the anti-predictive tail, D63) apply only until calibration; calibrated curves land as versioned config rows with the archived report. **v1.9.42 (Change 3, finding 271):** the flat-anchor sub-25th Suspect now requires SUSTAIN exactly as the trajectory does — a single dip below the 25th is **Warning**, only a sustained streak (`Monitor.FlatAnchorSustainEvals`, default 3) is Suspect — so a merely edgeless strategy's within-null excursions below the anchor are never flagged (the D63 invariant below). This is the pre-calibration fallback the two-pass calibration itself runs on, so it must be D63-conformant BY the rule text, not by whether the calibration gates go green. The panel plots the strategy's percentile *path* against both curves, so "too early for this band to mean anything" is visible rather than punished.
...
Rolling-window (63d) net alpha trend vs the strategy's own history **and vs its population band**: decaying toward/into the band is the signature of a dead edge. ⚙ Inside the band's central 50%: **Elevated** (Warning) at two consecutive windows and it **caps there — never Suspect** (v1.9.42, Change 3 / finding 271: the earlier "critical: three ⇒ Suspect" was the exact mid-band-lifer catch the scope note below forbids, and the spec was self-contradictory). The anti-predictive arm — a negative rolling alpha t < −1 **sustained** — is Warning once and **Suspect** only at `Monitor.FlatAnchorSustainEvals` (default 3) consecutive, so a within-null two-window excursion no longer trips it. Four consecutive Suspect evaluations ⇒ auto-retire (§2). **Scope note (D63):** S6 catches decay *from* an apparent edge and anti-predictive drift; a strategy that has simply never separated is the separation state's job (MASTER §20.8), not S6's — do not tune S6 to "catch" mid-band lifers.
