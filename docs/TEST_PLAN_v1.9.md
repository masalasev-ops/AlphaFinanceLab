...
- `FR37_ArenaNamespacedDbPath` — two configs differing only in `Arena.Id` resolve distinct absolute DB paths under `<DbBase>\{Arena.Id}\` (the assertion is base-agnostic — it owns the arena segment + token resolution, never the base, so relocating the DB per DB_RELOCATION.md cannot redden it); the bare EF design-time factory (no config) defaults to the `sp500` path (D71).
- `FR37_ArenaRegistry_DrivesClientBaseUrl` — the Web client's `ReadModelClient` targets the active arena's registry `baseUrl`; with one entry the first entry is active by default (D71).
...
- `Config_ConnectionString_EqualsResolverDefault` (Api + Worker) — the three connection-string spots agree (`ConfigConsistencyTests`; DB_RELOCATION.md).
