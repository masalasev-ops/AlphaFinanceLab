...
| `FX-ReproduceDay` | three committed forward sessions on a real migrated store; siblings with (a) one prior-session bar perturbed in the scratch copy, (b) a frozen held position carried across a session boundary, (c) no committed run for the target day | FR-25/NFR-1 (v1.9.37): `reproduce-day` re-runs the committed session from its stored watermark into a rewound scratch copy and the decisions, fills, equity points and population draws are **byte-identical**; the perturbed sibling **diverges** (a check that only ever passes proves nothing); the frozen sibling reproduces `frozen`+`frozen_reason` verbatim into `stage_json` from the D90 snapshot; the missing-run sibling fails closed; the live arena is provably unmodified (D59); the rewind guard throws on any surviving at-or-after row, and the table classification is total over the model so a future migration reddens until classified |
| `FX-RestoreThenContinue` | two sessions committed → per-launch backup → a third session committed → the backup copied over `alphalab.db` with the `-wal`/`-shm` sidecars removed | FR-25 (v1.9.37) / RUNBOOK §4: the restore discards exactly the post-backup session, the resolver reports it missed, catch-up replays it, `runs` is gapless, the equity curve matches its pre-restore shape, and `position_snapshots` is rebuilt so the restored store is reproducible again; a second launch is a no-op (FR-7 idempotency across a restore boundary) |
...
| **`FX-TwoPassCalibration` (v1.9.42, D100/D101)** | D64 plants under a calibration replay; hand-built P_noise curve + synthetic validate paths | findings 270–273: a plant is never actually Retired (stays promotable, emits S3 for the full window) but the would-be retire is logged with its triggering signal; `would_be_edge_survival` reads that log; `noedge_curve_breach_validate` / `curve_based_edge_survival` (their OWN keys) re-score the held-out validate segment with the sustain logic; `PrimaryEdgeIds` rule-selects the smallest MONTHLY rung clearing the offline floor (daily is survival-only — IN `FloorEdgeIds`, OUT of `PrimaryEdgeIds`); flat-anchor S3 needs SUSTAIN and S6 inside-band caps at Warning (OVERFITTING_MONITOR §3 D63 conformance). Tests: `Change1_ReplayPlant_ExemptFromRetire_*`, `Change2_CurveBasedMetrics_*`, `Change4_PrimaryEdgeIds_*`, `S3_FlatAnchors_*_OnlyWhenSustained`, `S6_NegAlphaSuspectOnlyWhenSustained_InsideBandCapsAtWarning` |
...
- `FR25_RestoreDrill` — scripted: snapshot → mutate → restore → verify (rehearsed per RUNBOOK §4, logged in PROGRESS.md). Automated as `FR25_RestoreThenContinue_*` (fixture `FX-RestoreThenContinue`, §3); the operator rehearsal stays, because the test proves the *path* and only a real drill proves *your* backup files are readable on *your* disk.
- `FR25_ReproduceDay_*` — the NFR-1 proof (fixture `FX-ReproduceDay`, §3): byte-identical on a committed day, divergent on a perturbed input, frozen state carried through the D90 book, read-only against the live arena.
- `FR25_VerifyWal_*` — `verify-wal` passes on a WAL store, fails closed with a named reason on a `journal_mode=delete` store, and **never sets** the pragma (a verifier that repairs what it checks can never report the defect it exists to find).
- `D90_PositionSnapshot_*` — the end-of-day book round-trips every field including `frozen`/`frozen_reason`, is idempotent per (account, session, run_kind) and drops names that left the book, quarantines replay from forward via the PK, and returns empty (never throws) for a session with no snapshot.
- `FR25_BackupOffsite_*` / `FR25_RegisterNightly_*` — the ops scripts actually run: the off-site copy picks the newest backup **by filename date** (not mtime), verifies size + SHA-256, and fails loudly with no destination or no backups; the nightly emitter produces a well-formed daily 02:00 task that launches the Worker **OnDemand** (never `--serve`, which would idle) and installs nothing without `-Install`. Skipped when no PowerShell is on PATH; both CI legs have `pwsh`.
...
- **OS-agnostic path resolution (v1.9.36, finding 218)** — six cases pinning the "one config string, every OS" contract that makes a cloud lift-and-shift a config-value change with no code change. `FR37_ResolvePath_NormalizesSeparatorsToTheRunningOs` — a template mixing `\` **and** `/` resolves with NO foreign separator surviving. The assertion reads the **`ResolvePath` output**, never `GetDataSourcePath`'s: the latter applies `Path.GetFullPath`, which on Windows normalizes `/`→`\` **by itself** and made the first version of this test pass with the production normalization deleted (caught by the v1.9.36 self-review; a single-separator template is likewise native somewhere and cannot fail there). `FR37_ResolvePath_LeavesUriDataSourceUntouched` — `file:…?mode=ro` data sources are URIs, not paths, and keep their `/`. `FR37_RequireAbsoluteStorePath_RejectsARelativeStore` + `FR37_Resolve_RefusesToCreateAStoreUnderTheWorkingDirectory` — a relative `Data Source` fails closed (rule 10) instead of giving Worker/Api/Backfill one empty DB each under their own CWD, and the writer refuses **before** creating a directory. `FR37_TheShippedDefault_IsWindowsAnchored_AndFailsClosedElsewhere` — the committed `E:/…` value is absolute on Windows and *deliberately* throws on POSIX (normalization makes the string parse everywhere; it does not make a drive letter meaningful), so a lift-and-shift that skips the four-spots edit fails loudly. `FR37_ThePortableTokenForm_IsAbsoluteOnEveryPlatform` — the `{LocalAppData}` form needs no per-OS edit. The **`ubuntu-latest` CI leg** exists for these: asserting an OS-agnostic claim only on `windows-latest` proves nothing about the half that matters.
