...
unused: wiring it is the S&P 500 widening *mechanism* — a recorded proposal (open in PROGRESS), not a flag you flip today. **Since D154 the Worker says so in code rather than only here:** `MembershipComposition.TryCreate` returns `null` for any universe but `sp100`, so an sp500 flip registers no membership provider at all, `MembershipRefreshStep` takes its "no provider composition" branch, and the roster's freshness goes visibly stale — instead of fetching ~101 OEF names against the `[495,510]` count band and failing closed with a data-shaped error for an unwired-code cause. D154's row lists the three things the sp500 arm still needs (a `Backfill:WikipediaSp500Url` key, evidence the cross-check parses the 500 table, and the `Source` label moving off `Bootstrap.`). There is no widening beyond the S&P 500: **D109** supersedes D87 — additional breadth is a **separate arena** (its own store, Worker+Api pair and calibration), never an enlargement of this one.
