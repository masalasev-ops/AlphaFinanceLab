...
| KPI | Measured how |
|-----|--------------|
| **Verdict honesty** | Every rendered comparison carries a correct, autocorrelation-corrected MDE; no gap **at or inside** its MDE is ever presented or acted on as a verdict (D146 settled that "inside" is `<=`: a gap exactly equal to its MDE is inside it, and a `NaN` on either side adjudicates nothing — the strict `<` rendered the degenerate never-traded pair, gap 0 against MDE 0, as a directional `Refused`) |
...
| Market & reference data | **EODHD (primary, D35)** behind `IMarketDataProvider` (raw+adjusted bars, splits/dividends), `IIndexMembershipProvider` (current + historical constituents), `IReferenceDataProvider` (sector/industry), `INewsProvider` (news). **Alpaca (free)** retained as bar cross-check/fallback. **iShares OEF holdings CSV** is the WIRED forward membership primary, with a Wikipedia S&P 100 cross-check; the IVV CSV is the S&P 500 arm and is deliberately unwired (D154 — `Ivv()` has zero production call sites). See §13 |
...
flowchart TB
    ...
    IM["IIndexMembershipProvider (EODHD)<br/>+ OEF CSV primary + Wikipedia sp100 cross-check"] --> STORE
...
- **Stage 5 sizes new opens against available CASH, not equity (D84):** a day's opens can never total more than the cash on hand — they scale to fit, a near-zero-cash day opens nothing, and no held name is ever sold to fund an open (rule 7). A whole-book rebalance re-weights the TRADABLE book against equity instead (it self-funds); a §13.6-frozen position is excluded from sizing entirely and is never traded on a rebalance (D147 — it has its own list on `PortfolioPlan`, so `ToSize` cannot reach it). `Sizing.PositionCapPct` still binds per name.
...
- **Enforced budget & graceful degradation (D24, as amended by D130):** every spend cap DERIVED from the one authored `Llm.AnnualBudgetUsd` — the cost cap is no longer authored. Hard daily ceilings (tokens/calls/cost) that **accumulate across a batch** (D151, finding 420): before it only the call count did, so an N-request batch could cross the cost and token caps by N−1 requests. Cache hits free and contribute nothing to the running total. On exhaustion, degradation is **currently ARRIVAL order** — the D24 priority contract (held positions first, then cached, then neutral fallback) is read by nothing in `src` and is recorded as unimplemented (finding 421, deferred). Never a blackout.
...
| Index membership (current + history) | **EODHD** constituents (GSPC.INDX; historical snapshots since 2000) | **WIRED (sp100): iShares OEF holdings CSV primary + Wikipedia S&P 100 cross-check**, count sanity 99–103; divergence alert. The IVV/`[495,510]` sp500 pairing is the UNWIRED arm — `CountBandFor` does switch on the token, so a flip would move the band while the providers stayed put, which is the mismatch D154 was written about | Wikipedia scrape |
...
- **Universe** = the full set with local data. **Start: the S&P 100 slice through Phase 4 sign-off (D65/D70 — sourced from the iShares OEF holdings CSV with a Wikipedia S&P 100 cross-check). WIDENING TO THE S&P 500 IS NOT A CONFIG FLIP TODAY (D154): `sp100` is the only wired universe, and any other token registers NO membership provider, so the roster goes visibly stale rather than fetching ~101 OEF names against the `[495,510]` count band. The widen needs D154's three items — a `Backfill:WikipediaSp500Url` key that exists nowhere, evidence the cross-check parses the S&P 500 table, and the cross-check `Source` moved off `Bootstrap.` or the rows carry false provenance (D137) — plus the backfill delta. That is where this arena's universe STOPS — D109 supersedes D87: breadth beyond the S&P 500 is a SEPARATE ARENA (D71/rule 23), never an in-place widen.** History backfilled for all. **Arena Replay is the standing exception (D70): replay always runs on S&P 500 as-of membership, with bars backfilled for every historical member in the replay window before Phase 4 begins.**
...
- Membership refresh (daily): pull EODHD constituents → **cross-check vs the OEF CSV + Wikipedia S&P 100 pair** (the wired composition; D154) → on agreement, flip `in_index` + stamp dates on the **security** row; on divergence, alert and hold yesterday's state (fail closed). An unwired universe token registers NO provider at all, so the refresh reports "no provider composition" and freshness goes visibly stale rather than guessing a roster. **Never delete rows or bars.**
...
- Corrections arrive as **new versions**: insert `(security_id, date, version+1, observed_at)`. No UPDATE, no DELETE — CI greps cover the raw-SQL forms **and**, since D151, the EF idiom this codebase actually writes (guard 1d, table-qualified over `Bars`/`CorporateActions`/`Config`), because the SQL patterns alone had never had anything to match.
...
Unchanged five disciplines — ACID per daily run, WAL, nightly file-copy backup, snapshot-before-migration, and no-delete enforcement — extended by D40: **no `UPDATE bars` either**; corrections are insert-only versions. **What enforces it (D151, finding 418):** the raw-SQL greps this line used to name in isolation matched a form appearing NOWHERE in the repo, while the codebase performs DML through EF at 43 sites — so `ci.ps1` guard 1d matches the EF idiom, table-qualified over the three append-only tables, guard 1e self-tests that it fires and stays silent on the sanctioned `ExecuteDelete` calls, and the SQL patterns are kept beside it for a future raw-SQL migration. KNOWN GAP: the EF pattern is single-line.
...
3. **Clamps, in order:** per-strategy floor/ceiling (`WeightFloorPct`/`WeightCeilingPct`, defaults 5%/60%) → `TooEarly` against the benchmark (D131) ⇒ |t_i − current_i| ≤ `TooEarlyTiltCapPts` → Suspect ⇒ t_i = current_i × (1 − `SuspectDecayPctPerEval`), no new tilt → movement only if |t_i − current_i| > `BandPts`, and then **to the band edge**, not to t_i (banded, slow) → renormalize. Baselines and control populations never receive weight. **Floor feasibility (v1.9.7, finding 116):** floors apply pre-renormalization and scale down proportionally whenever Σfloors would exceed 100% (equivalently, the promotable roster is capped at ⌊100 / `WeightFloorPct`⌋ — 20 at the default), so the clamp chain is never asked for an infeasible vector; renormalization never pushes a floored weight back below its (scaled) floor. **What "a floored weight" means, and the asymmetry with the ceiling (D150, finding 416):** the guarantee binds the rows the floor clamp actually PRODUCED — those whose final `applied` still IS the (scaled) floor — and deliberately not those a later clamp moved off it, because lifting a Suspect-decayed row back to the floor would reverse clause 3's "no new tilt". Renormalization pins those rows and spreads the remainder pro rata; feasibility is inherited from the ⌊100 / `WeightFloorPct`⌋ cap above, so the remainder can never be negative. **There is NO symmetric ceiling guarantee and its absence is deliberate:** the clamps are step 3 and the renormalization is step 5, so a post-renormalization weight may exceed `WeightCeilingPct` (a 2-name roster under a 60% cap cannot sum to 1 otherwise), and a fixture pins that so it is not "completed" by a later reader.
...
- **Stage 1 — fetch (no DB writes):** all provider calls; raw payloads archived to `tools/raw-cache/{source}/{date}/`; the FR-6 quality gate validates *staged* data; a hard failure aborts before a single row is written (tomorrow's catch-up recovers). **Which failures abort is severity- AND difference-aware (D145)**, not date-only: a `warn` on an already-gated date is dropped (the fetch window carries a ~40-session context tail), while a `reject` survives when the fetched bar DIFFERS from the stored latest-visible one — `BarIngestionService.Differs`, the writer's own definition — or when no stored bar exists. An identical re-fetch of a gate-rejectable stored bar is an idempotent no-op rather than a permanent wedge; P7 stays open for the ~488k never-gated backfilled bars.
...
- Computed in `AlphaLab.Evaluation` read-model builders — never in the UI (D58); the band and track-length knobs are config under `Verdicts.*`, but the `distinguishable` arm's EDGE BAR and its sustain count are read from the row the monitor itself wrote (`threshold_json.p_edge_at`, or `healthy_anchor` pre-calibration, plus that row's `sustain_evals`) so the monitor and the read-model cannot judge the same strategy on the same day against different numbers — deliberately not a second knob (D148). Days-to-statement for a no-edge plant is a §1.2 KPI, measured in replay.
...
`AlphaLab.Api.Tests`: each read endpoint returns the declared read-model shape **stamped with run_id+watermark**; a create without hypothesis-or-`unregistered` returns 422; an admin action without a matching token returns 422; a command during `run_in_progress` returns 409; `POST /replay` and `POST /analysis/*` return 202+job_id (not a blocking result); a budget-exhausted analysis call returns 503; a `POST /analysis/skeptic` with no `strategy_id` returns 422 before the budget check while an arena-level `POST /analysis/brief` still dispatches (D153); money fields serialize without floating-point.

## 22. Read-models — where the honesty lives (D58)

*Plain serializable DTOs produced by `AlphaLab.Core`/`AlphaLab.Evaluation`, consumed by `AlphaLab.Api`, rendered verbatim by any UI. The honesty rules (UX-1…UX-20) are resolved into fields here so no client can violate them.*
