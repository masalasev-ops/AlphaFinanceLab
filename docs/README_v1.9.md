...
2. **Get the repo.** Clone the repository. The layout is already in place, **nothing to copy or rearrange**: `CLAUDE.md` + `PROGRESS.md` at the **root**, the design set (all `.md`) and the mockups (`.html`) under `docs/`. The reference build has progressed through Phase 4 (Arena Replay, signed off) with Phase 4.5 code-complete (see `PROGRESS.md`); for a from-scratch rebuild see `docs/REBUILD.md`.
