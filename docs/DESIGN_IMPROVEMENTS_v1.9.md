...
- **The construction haircut, and why the number is now MEASURED (D122):** transaction costs, the missing short leg, large-cap-only universe, and capacity all compound, so a long-only large-cap tilt keeps only a FRACTION of a published long-short premium — and some years are negative. **This document no longer asserts what that fraction is.** An asserted constant was defensible while it was the only number available; it is derived from ONE construction, and a lab that may change its construction cannot calibrate everything to it. The arena now measures its own detectable band and publishes it in the frozen `Calibration.DetectionPower` row (sp500, generation 2: **[6.95 %, 32 %]/yr** at the D121 horizon). §6's power math reads that row; a different construction produces a different band by measurement.
