...
> **D49 launch configuration (SETUP_v1.9 §1):** on the All World tier, §1's constituents/sector/fundamentals rows are **dormant** — membership runs OEF-CSV-primary + Wikipedia-S&P-100-cross-check (the wired pair; the IVV/sp500 arm is unwired - D154), sectors come from the IVV CSV's GICS column, and replay's as-of membership seeds from the fja05680/sp500 community CSV (§8). The rows remain specified here so the post-upgrade flip is a config change (`Universe.MembershipPrimary` etc.), not new integration work. **D70:** the S&P 100 launch slice is itself a named feed — see §2b — and replay never uses the slice (S&P 500 as-of membership only).
...
- **Rejected on the Batches API — do not design around it:** the server-side `fallbacks` parameter. A refused or failed scheduled read degrades **never through a fallback model** — that half stands. The D24 `DegradationOrder` half does NOT: it is read by nothing in `src`, and since D151 made mid-batch refusal reachable at all, refusal is ARRIVAL-ordered (finding 421, open).
