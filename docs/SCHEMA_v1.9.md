...
- **CI greps:** `DELETE FROM bars` / `UPDATE bars` in `src/`, `tests/` or `tools/` fail the build (guard 1) — and, since **D151** (finding 418), so does the EF idiom this codebase actually writes: `Bars`/`CorporateActions`/`Config` followed by `Remove`/`RemoveRange`/`ExecuteDelete`/`ExecuteUpdate`, scoped to `src/` + `tools/` (guard 1d). The raw-SQL patterns had **never had anything to match** — those strings appear nowhere in the repo — while 43 EF DML sites did, so "CI greps enforce" described a check that could not see the writes it forbade. The EF pattern is single-line and table-qualified; both limits are stated in `tools/ci.ps1` and a self-test (guard 1e) proves it fires on a synthetic violation and stays silent on the legitimate `ExecuteDelete` calls in `ReplayRunner`/`ScratchStore`.
