...
- **v1.9.7 finding 108 — `config` is append-only-versioned:** the PK is `(key, version)`; the current value is `MAX(version)` per key; a change INSERTs `version+1` and never UPDATEs or DELETEs an existing row. The Phase-4 calibration curves (D56/D63) rely on this to keep their version history.
