...
*Companion to the four mockup files (`allocation_journal_ops_ux_mockups.html` covers UX-9…UX-11). **These rules are enforced in the serializable read-models (D58), not in any UI framework** — each rule below is resolved into DTO fields in `AlphaLab.Core`/`AlphaLab.Evaluation`, served by `AlphaLab.Api` (D57), and rendered verbatim by whatever client is attached (Blazor today; Angular/React/mobile equally). The mockups show the reference Blazor client; the rules hold for all clients because a client cannot recompute them.*

*Original companion note:* The v5 mockups (`paper_trading_ui_mockups.html`, `decision_and_strategies_mockups.html`) set the visual direction — palette, gold-means-live, plain names, health pills, MDE line. The v6 mockup (`lab_honesty_ux_mockups.html`) fixes the UX contradictions the design review surfaced. This doc states each rule, why it exists, and where Claude Code implements it. Phases 3 and 7 cite these as **UX-1 … UX-13** (UX-1…UX-8 originally; UX-9…11 added in v1.8, UX-12 in v1.9, UX-13 in v1.9.3); read-model tests in TEST_PLAN §8 enforce the testable ones.*

**UX-13 — Arenas never merge (D71c).** When more than one arena exists, the UI selects one **active arena** and every screen renders that arena only, with a provenance line naming its control population, cost-model version, and calibration id. A cross-arena view is permitted **only** as clearly-separated side-by-side panels, each with its own provenance and its own sort — **never a single sorted ranking that mixes arenas**. Ranking a strategy from one universe against a strategy from another compares numbers produced under different cost models and different control populations — the same category error UX forbids between cadence families. There is no "sort all strategies across all arenas" control. *(v1.9.3; full spec in ARENA_ARCHITECTURE_v1.9.3 §4. Specified in text only — none of the four mockups depicts the arena switcher or the provenance line; all predate D71. Follow this rule's text, not the mockups' absence.)*
