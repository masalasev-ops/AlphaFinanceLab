...
*Original companion note (historical):* earlier per-topic mockups set the visual direction — palette, gold-means-live, plain names, health pills, MDE line — and a later honesty pass fixed the UX contradictions the design review surfaced. All of that is now consolidated into `alphalab_ux_mockups.html`. This doc states each rule, why it exists, and where Claude Code implements it. Phases 3 and 7 cite these as **UX-1 … UX-16** (UX-1…UX-8 originally; UX-9…11 added in v1.8, UX-12 in v1.9, UX-13 in v1.9.3, UX-14 in v1.9.22, UX-15 in v1.9.34, UX-16 in v1.9.52); read-model tests in TEST_PLAN §8 enforce the testable ones.*
...
### UX-15 - the cohort maturation panel: honest at every age (v1.9.34, D88)
**Rule:** the Analysis/Journal research surface renders the cohort maturation curves verbatim from `CohortMaturationReadModel` (FR-39): one line per admission cohort, median D36 population percentile vs track length t in trading days, age-aligned, retired members retained. The panel must render the thin-cohort dimming (`display=="dimmed"`, reason `thin_cohort`) and the sub-MDE dimming (reason `inside_mde`) exactly as the read-model resolves them (the UX-1 MetricCell convention - the panel never computes dimming), and replay cohorts (`quarantined:true`, e.g. the D64 plant cohorts) sit on the UX-8a replay field with its watermark and are never co-plotted with forward cohorts. A cohort-to-cohort gap smaller than its MDE is never presented as an improvement. Disambiguation: the S3 percentile panel on Overfitting health is a single strategy's path against its band; the cohort maturation chart on Analysis is a median across an admission vintage; the two share the percentile source and are never visually merged. The panel wears a PlannedBadge until Phase 3 wires the read-model. *(Reference look: `docs/mockups/cohort_curve_panel.html` - a standalone illustrative panel; the consolidated `alphalab_ux_mockups.html` absorbs it when the UI workstream next regenerates.)*
**Why:** the curve answers "is the researcher loop learning what to test, or just recombining?", and every flattering shortcut (dropping dead members, comparing unequal ages, reading a sub-MDE gap as progress, letting plant cohorts pose as forward evidence) is resolved into the read-model, not left to the chart.
**Where:** Analysis/Journal screen (Phase 3 read-model; rendered wherever the UI workstream lands per D65); component `CohortCurvePanel` (UX_DESIGN_SYSTEM 17); test `UX15_CohortCurve_ThinAndSubMdeDimmed_ReplayNeverCoplotted`.

### UX-16 - the signal library panel: a grade is never shown without the sample behind it (v1.9.52, D91/D108)
**Rule:** the Analysis/Journal research surface renders the Signal Library verbatim from `SignalLibraryReadModel` (FR-46): one row per (signal, horizon), each carrying both rolling windows (1y and 5y mean rank-IC with their Newey-West bands) and ONE trend flag, inferred on the 5-year window for both horizons (D108). **The effective independent sample (`effective_n`) renders beside every flag, always** - it is an INPUT to the verdict, not a footnote: it sets the degrees of freedom and therefore the critical value. A row whose effective sample is below the floor renders `flag="insufficient"` with its `flag_reason`, and the panel must show that state as its own thing - **never as "stable", and never as a blank that reads like agreement**. The mean is still shown in that state; what is withheld is the significance claim. `t_stat`, `level_critical` and `trend_critical` are rendered on the row (or its hover) so a reader can recompute the verdict rather than trust it - the panel never computes a flag, a band, or a dimming (the UX-1 MetricCell convention). Disambiguation: this panel grades RULES; the Strategies screen grades TRADING OPERATIONS. They are never merged, and the read-model's `detection_context` line renders beside the panel to say why - a signal can grade well and still describe an edge the arena could never confirm. The panel wears a PlannedBadge until the UI workstream renders it (deferred per finding 293).
**Why:** the instrument's whole value is telling you when a published rule has stopped working, and every flattering shortcut runs the other way - reading a four-observation window as evidence, showing a flag without the sample it rests on, letting "we cannot say yet" render as "fine", or quietly converting a good IC into an implied arena edge. Each is resolved into the read-model rather than left to the chart.
**Where:** Analysis/Journal screen (Phase 4.5 read-model; rendered wherever the UI workstream lands per finding 293); component `SignalLibraryPanel` (UX_DESIGN_SYSTEM 18); test `FX-SignalPanel`. *(Reference look: `docs/mockups/signal_library_panel.html` - a standalone illustrative panel; the consolidated `alphalab_ux_mockups.html` absorbs it when the UI workstream next regenerates.)*
...
## Phase wiring
Phase 0: empty-state copy framework (UX-8c). Phase 3: UX-1, UX-2, UX-3 (minimal), UX-4, UX-5 (minimal), UX-6, UX-10's pre-registration modal, UX-9 minimal, UX-12 (read-model; FR-35), UX-15 (read-model; FR-39). Phase 4: UX-8a, UX-11b. Phase 4.5: UX-16 (read-model; FR-46). Phase 6: UX-5 (full). Phase 7: UX-3 (full), UX-7, UX-8b, UX-9 full, UX-10 full, UX-11a/c. UX-13 carries no phase obligation in the single-arena build — it binds the moment a second arena is registered (a future config + instance operation, ARENA §6); the calibration-provenance line may ship any time after Phase 3 alongside UX-4's chips. Screens may lag per D65 (API-only (Scalar) sanctioned until Phase 4 sign-off; hard deadline Phase 7 exit) — read-models and their tests never lag. TEST_PLAN §8 gains `UX1_InsideMde_MetricCell_IsDimmedWithTilde`, `UX8_ReplayNeverCoplotted`, `UX12_SeparationChip_RendersWhenTrackExceedsMinAndStateNone`, and `UX15_CohortCurve_ThinAndSubMdeDimmed_ReplayNeverCoplotted`.
