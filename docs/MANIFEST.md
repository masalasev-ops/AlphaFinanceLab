...
**Build & test**
- `docs/BUILD_AND_PROMPTS_v1.9.md` — FR-1…FR-47, the gated phase plan, and the ready-to-paste
...
- `docs/UX_DESIGN_SYSTEM_v1.9.md` — the component catalogue: each honesty read-model field → its Blazor component, element, and token treatment (entries 1–19). The visual-assembly layer under UX_GUIDELINES' tokens and UX-1…UX-20.
...
- **v1.9.94 – v1.9.113 — Phase 6 (checkpoints 6.1–6.5a)**, compressed here on purpose: `docs/CHANGELOG_v1.9.md` carries these twenty passes finding-by-finding and is the authority; this trail records only that they exist, because a per-pass restatement is the duplication finding 332 was raised about. **D131–D154** landed (register: **154 rows, D1..D154**; next free finding **428**). The arc: 6.1 scope + the D131/D132 corrections; 6.2 the row-driven strategy registry; 6.3/6.4 the D133–D141 lifecycle, membership provenance and calibration-read work; **6.5** the S6 remedy (D138/D139) and the D117 confirmation slice; and **6.5a**, a thirteen-PR review-remediation block closing eighteen confirmed findings — D142/D143 (a corporate action reaches PENDING orders: restate, then cancel on a terminal event), D144 (a generation records the ledger arithmetic that produced it and refuses a mixed re-run), D145 (Stage 1's re-gate becomes severity- and difference-aware), D146 (rule 6's test is `<=`, and a NaN MDE adjudicates nothing), D147 (the §13.6 freeze rail: not sized, not traded, never cleared by a fill), D148 (SeparationState reads the monitor's own bar; both S3 arms require a streak), D149 (forward visibility becomes one seam), D150 (renormalization becomes floor-aware), D151 (three guards that could not catch what they forbade), D152 (what executes is reconciled with what was frozen — rule 8's first enforcement), D153 (the skeptic is given a subject and the arena's numbers), D154 (an unwired universe registers no provider). Six items are recorded OPEN rather than fixed: P25, P26 and findings 417, 421, 424, 427.
