...
```jsonc
{
  ...
  // t_{1-alpha, df} computed at read time, where df comes from the effective independent sample
  ...
  // BOTH ARMS ARE ONE-SIDED - hence t_{1-alpha, df} and not t_{1-alpha/2, df}: both claims are
  // directional ('gone' = the mean not significantly ABOVE zero; 'decaying' = the slope significantly
  // NEGATIVE). The two-sided form appears only in the REPORTED CONFIDENCE BAND on each rolling window
  // (SignalLibraryBuilder) - that is an interval, never a flag threshold (finding 302).
  ...
  // "MinDetectablePower": 0.80             // config ROW. The detection power the published
  //     minimum-detectable-IC is quoted at (finding 305). Derived from Gate.Power = 0.80, the same
  //     power D48's MDE uses. OPTIONAL, and the asymmetry with the two alphas is deliberate: the
  //     alphas gate a VERDICT and the backfill refuses to grade without them; this scales a published
  //     DIAGNOSTIC, so an absent row withholds the floors with a stated reason and never blocks a run
  //     or changes a flag. The floor itself is NOT stored - it is (t_{1-alpha,df} + t_{power,df})*se
  //     computed at read time, because both df and the standard error depend on the window available.
  //
  // WRITTEN BY the Worker verb `signal-pin-thresholds` (finding 299) - the ONLY sanctioned way to create
  // these two rows; a hand `sqlite3` INSERT is a rule-15 violation:
  //     dotnet run --project src/AlphaLab.Worker -c Release -- signal-pin-thresholds [--arena <id>]
  //         --gone-alpha <alpha> --decay-alpha <alpha> [--power <p>]
  // Both alphas are REQUIRED and explicit - a missing one is REFUSED, never defaulted to 0.05 (a silent
  // default would defeat the pin-before-grade discipline, which exists to make the operator state the
  // value deliberately). A value outside 0 < alpha < 1 is refused rather than clamped. An ALREADY-pinned
  ...
  "Llm": {                                         // D24/D46
    ...
    "NewsBudget": { "MaxArticlesPerRead": 25, "MaxCharsPerArticle": 2000,
                    "DedupeBy": "title_hash" },
                                                   ...
                                                   // silently. It STILL FAILS CLOSED on a model matching no configured prefix -
                                                   // the relaxation is from "exact key" to "known family", never to "anything".
  ...
  "Research": {                                    // ~~COMMITTED IN src/AlphaLab.Api/appsettings.json ONLY (v1.9.67): ... The Worker reads Ai.*, never Research.*~~ CORRECTED (v1.9.70, finding 334): committed in BOTH src/AlphaLab.Api/appsettings.json AND src/AlphaLab.Worker/appsettings.json, held equal by Config_Research_AgreesAcrossProcesses. The v1.9.67 claim inverted the moment the researcher pack was actually wired: fork_budget_remaining (the cp-1.0 pack field) is computed Worker-side from ForkBudgetPerYear. The Api consumers (the D112 gate, the 202 budget surface) are unchanged
...
7. **v1.9.52 (D108) — the Signal-Library trend thresholds are config ROWS, not appsettings keys.** `SignalLibrary.TrendDecayAlpha` and `SignalLibrary.TrendGoneAlpha` are written once as versioned `config` rows at build checkpoint 4.5.2, **before the first `signal_ic` row exists**, under the same append-only-never-re-stamped discipline as the D98 calibrated keys. Rows rather than appsettings because the FR-46 read-model must resolve them through `ConfigReadService.ResolveAsOf` to serve the watermark-pinned Phase-5 digest (finding 292), and an appsettings value is not as-of resolvable. Each holds a **significance level**, never a critical value: the critical value is `t_{1−α, df}` computed at read time from the effective independent sample (`df = n_eff − 1` level arm, `n_eff − 2` trend arm). Both arms are **one-sided** — `gone` tests that the mean is not significantly *above* zero, `decaying` that the slope is significantly *negative* — so the read-time value is `t_{1−α}`, never `t_{1−α/2}`; the two-sided form is used only for the reported confidence band on each rolling window (`SignalLibraryBuilder`), which is an interval, not a flag threshold (finding 302). Renamed from `TrendDecayZ`/`TrendGoneZ`, which named the normal-reference z that D108 replaced. The FR-45 backfill **refuses to run** while either row is absent (fail closed, logged reason) — that refusal, not a comment, is what enforces pin-before-grade. A THIRD row, `SignalLibrary.MinDetectablePower` (finding 305), is written the same way but is **optional and never gating** — it scales the published minimum-detectable-IC, a diagnostic, so its absence withholds that number with a stated reason rather than blocking a run. The rows are created by the Worker verb `signal-pin-thresholds` (`dotnet run --project src/AlphaLab.Worker -- signal-pin-thresholds [--arena <id>] --gone-alpha <α> --decay-alpha <α>`, finding 299) — the **only** sanctioned writer; a hand SQL `INSERT` is a rule-15 violation. Both α values are explicit (a missing one is refused, never defaulted to 0.05), a value outside (0,1) is refused rather than clamped, and an already-pinned key is left untouched and reported — pinned once, never re-stamped, with the derivation recorded in each row's `reason`.
8. **v1.9.12 binding caveat (finding 159):** some blocks documented here are the *designed* surface for a later phase and are **not yet section-bound**. `CalendarOptions`, `RegimeOptions`, and `DataQualityOptions` declare a `SectionName` (`Calendar`/`Regime`/`Data`) but are currently DI-registered as **default instances** (not `GetSection(...).Bind`), so a value placed in those sections is silently ignored until the phase that consumes them wires the bind. No value drift today — the defaults equal the documented defaults — but treat these as compile-time constants, not live knobs, at the current phase. (The `Universe.Bootstrap` binding gap is tracked separately under **D76** — finding 151.) The `Kpi` block (D88, v1.9.34) was listed here through v1.9.33 and is **no longer a designed surface**: the Phase-3 read-model build landed `KpiOptions`, section-bound in the Api composition root via `GetSection(KpiOptions.SectionName).Get<KpiOptions>()` and consumed by `CohortMaturationBuilder`, with its defaults tripwired against this file by `Phase3OptionsDefaultsTests`. Values placed in `Kpi` are live. (`SignalLibrary` is likewise section-bound — in the Api composition root and in `SignalBackfillRunner` — and its two α thresholds are config ROWS, outside this caveat entirely per key rule 7.)
