...
- **How small an edge the lab can see is MEASURED, not assumed (D122).** There used to be a number here — a stated guess at the realistic prize — and every margin of error was tuned to it. That guess came from one particular way of running the lab (buy-only, large-cap, these costs), so it stopped being the right anchor the moment that could change. The lab now runs its own calibration and reports the band it can actually adjudicate; today that is roughly **7% to 32% a year**. Change how the lab trades and the band is re-measured, not re-argued. **What has not changed: the edge is small and slow to prove, which is why patience is built into everything.**
