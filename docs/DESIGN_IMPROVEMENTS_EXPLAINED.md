...
- **How small an edge the lab can see is MEASURED, not assumed (D122).** There used to be a number here — a stated guess at how much the lab would earn — and every margin of error was tuned to it. That guess came from one particular way of running the lab (buy-only, large-cap, these costs), so it stopped being the right anchor the moment that could change. The lab now runs its own calibration and reports the band it can actually adjudicate; today that is roughly **7% to 32% a year**. Change how the lab trades and the band is re-measured, not re-argued. **What has not changed: the edge is small and slow to prove, which is why patience is built into everything.**
...
3. **Engineer comparisons to be tight.** Since near-identical comparisons can resolve in about a year (a derived, worked-example figure — the spec's own caveat: effects are measured, never asserted) while loose ones take decades, the whole design pushes toward tight comparisons: the AI contestant and its no-LLM twin differ in exactly one ingredient (the LLM decision layer — MASTER §23.3), parameter variants differ in exactly one setting, and the allocator tilts continuously instead of waiting for slow statistical certainty. Pairing tightness is the single biggest lever *you* control over how fast the lab can tell you anything.
