...
**The trap:** it's tempting to let an AI "pick stocks" or generate a magic sentiment score and trust it. That's exactly the kind of unfalsifiable black box the rest of the lab is designed to avoid.

**What we actually do**, and why:

- **Claude's main job is to be a research assistant and a skeptic, not a stock-picker.** It writes bull/bear briefs on names the *system* surfaced, narrates what regime the market seems to be in, generates hypotheses, and — most valuably — plays devil's advocate: you paste a strategy's stats and ask "what's the overfitting story here? how might this be fooling me?" It reads text; the *math* does the judging. This division is deliberate: language models are good at reading and arguing, and untrustworthy at being a hidden numeric edge.

- **Any "sentiment score" it produces is treated as experimental and put on trial.** It's only ever a minor ingredient, and it's tested by running two otherwise-identical accounts — one that uses it, one that doesn't. If the score is worthless, that comparison will show it, and *that's a valid finding, not a failure.* (Bonus: because those two accounts are nearly identical, this is the single fastest, cleanest experiment the whole lab can run — see §6.)
...
3. **Engineer comparisons to be tight.** Since near-identical comparisons resolve in ~1 year and loose ones take decades, the whole design pushes toward tight comparisons: the Claude A/B accounts differ in exactly one ingredient, parameter variants differ in exactly one setting, and the allocator tilts continuously instead of waiting for slow statistical certainty. Pairing tightness is the single biggest lever *you* control over how fast the lab can tell you anything.
