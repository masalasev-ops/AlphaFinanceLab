...
What: SCOPED DOWN (v1.9.91) — the digest-into-pack half of this item ALREADY SHIPPED: D113 wires the D91 signal digest into the researcher's pack as the evidence-prior seam's treatment field, and v1.9.70 wired and BLINDED the path (D114). What remains post-Phase-8 is the RICHER prior this item always meant (which hypothesis families cleared their MDE, at what track lengths, in which regimes, and which died) and, above all, the five guards below. The defining feature is not the learning, it is the guarding, because a learning researcher that is not guarded fits the lab's own noise and only appears to improve.
