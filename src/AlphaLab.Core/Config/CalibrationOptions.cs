...
public sealed class PlantOptions
{
    ...
    /// <summary>Regime multipliers on the PIT trend label (default bull 1.25 / bear 0.5), renormalized
    /// by the RUNNING realized regime mix so the unconditional annual target still nets to
    /// <see cref="AlphaAnnualPct"/> (PIT-clean: the normalizer uses labels ≤ t only).</summary>
    public double BullMultiplier { get; set; } = 1.25;
    public double BearMultiplier { get; set; } = 0.5;
