...
AlphaLab is a **paper-trading laboratory**. It trades no real money. Its purpose is not to run one clever strategy and hope it makes money, it is to run *many* investment strategies side by side against each other, in real time going forward, and to work out **which ones actually have an edge and which ones are only getting lucky**. Over time it uses those results to propose better strategies to try next. So it is two things at once: an experiment that judges strategies fairly, and a system that improves itself by learning from its own results. It runs on the S&P 500, and that is where this lab's universe stops. Extra breadth does **not** arrive by enlarging it: under decision **D109** (which supersedes D87) another market — say the S&P 400 mid-caps — would be registered as its own separate **arena**, with its own database, its own calibration and its own leaderboard, and results are never pooled between arenas. The reason is that one rule confirmed independently in two arenas is stronger evidence than the same rule measured once in a bigger sample.
...
## Under the hood: how the system grows, and where fundamentals fit

The previous section said the researcher can search its own space of ideas but cannot expand that space, and that adding genuinely new building blocks is a human job. This section explains the mechanical reason, because it answers a natural worry: does the system rewrite its own code as it runs? It does not, and the way it avoids that is worth understanding plainly.

**A strategy is a saved record, not a program.** The system has a small, fixed set of recipe types built into it: momentum, mean-reversion, low-volatility, and so on. Those are the compiled building blocks, and they change only when a developer ships a new release. A strategy, by contrast, is just a saved record that points at one of those recipe types and fills in its numbers. "Momentum, look back 126 days, hold 21, pick 40 names" is one record. "Momentum, look back 63 days, pick 20" is another record, using the very same compiled momentum recipe. So a strategy is data, not code.

**This is why the researcher can work continuously without any rebuild.** When the researcher proposes a new candidate and you approve it, all that happens is a new record gets written. No code is compiled, nothing restarts, no one redeploys. The daily run simply notices the new record the next morning and starts scoring it. Every variant the researcher will ever propose, every re-parameterization, every fork, is a new record over recipe types that already exist. That is what "the researcher searches its own space" means in practice: it writes records, it never writes code.

**The system needs a developer, and a new release, in exactly one situation:** when you want a mechanism that none of the existing recipes can express, or a kind of data the system has never had. A brand-new recipe type is a new piece of code that someone writes, tests, and ships. This is not a limitation the system is straining against, it is a deliberate boundary. If the system could invent and wire in its own new mechanisms while running, you could never honestly count how many different things it had really tried, and the whole statistical discipline that makes its verdicts trustworthy would quietly collapse. So the rule is simple and protective: the records change on their own all the time, the building blocks change only when a human ships them.

```mermaid
flowchart TB
    subgraph DATA["Changes on its own, all the time (data)"]
        P["Researcher proposes<br/>a new strategy"]:::notrade
        P -->|"a human approves"| REC["A new record is written<br/><i>an existing recipe, new numbers</i>"]:::machine
        REC -->|"no rebuild, no restart"| FIELD["It joins the field<br/>on the next daily run"]:::trade
    end
    subgraph CODE["Changes only when a human ships it (code)"]
        NEW["Add a brand-new tool<br/><i>a new recipe type, or new data like fundamentals</i>"]:::verdict
        NEW -->|"a developer writes, tests, ships"| REL["Available in the next release"]:::outcome
    end
    classDef trade fill:#E8F5E9,stroke:#2E7D46,stroke-width:1.5px,color:#14351f;
    classDef notrade fill:#EEEEEE,stroke:#7A7A7A,stroke-width:1.5px,color:#2b2b2b;
    classDef machine fill:#E7F0FB,stroke:#1F5FA6,stroke-width:1.5px,color:#0f2f52;
...
**Where fundamentals fit is the clearest example of this whole distinction.** Fundamentals means company financial data: how cheap a company is relative to its earnings, how profitable it is, how strong its balance sheet is. It is exactly a new building block, the deliberate human kind, and it arrives in three stages.

**Today, the system is price-only.** Every strategy, and every factsheet the AI trader reads, is built from daily price and volume. The financial-statement data is not wired in yet, so no strategy can currently look at how cheap or how profitable a company is. This is not an oversight, it is simply the current stage of construction.

**At Phase 8, fundamentals becomes a building-block expansion, done the human way.** It adds two new recipe types, value (buy companies that are cheap relative to their fundamentals) and quality (buy financially strong companies), and it wires in the financial data behind them. There is one hard requirement that this stage lives or dies on: point-in-time correctness. A company's quarterly numbers only become public weeks after the quarter ends, when the filing lands. The system must use only the figures that were actually public on the day it decided, never the latest restated version, or it would be reading the future, which is the single most common way this kind of strategy fools its builder. Once the data is wired in correctly, value and quality are judged by the exact same honest machinery as everything else, against the same random crowds and the same slow verdicts. From that point on, the researcher can propose value and quality variants as ordinary records, just as it does for the price-based recipes, because the building block now exists.

**After Phase 8, the wider fundamentals world is where the later improvements pay off.** A bigger and more varied search space is worth more when you also make the verdicts more true and search the space more cleverly. Two such additions are planned: a cost model that accounts for how hard it is to trade less-liquid companies at real size, and a researcher that learns from the lab's own results under strict guards so it proposes better over time. Both are documented separately in the two post-Phase-8 roadmap files, are deliberately not part of the core build, and build on the fundamentals expansion rather than replacing anything in it.
...
Every strategy in the lab is built on a prediction rule, for example "stocks that rose over recent months tend to keep rising." The arena tests whole trading operations built on those rules, with money, costs, and exits attached, and that is deliberately slow: each operation produces one portfolio number per day. The signal library, added as its own small construction stage (Phase 4.5, between the replay stage and the AI stage), grades the rules themselves. Each day, every registered rule ranks the eligible stocks **it can actually score**, using only information available that day: eligible means in the index that day and priced that day, and each rule then drops the names its own formula cannot rank — a rule needing a year of history omits a name with only six months, which a short-window rule keeps — so the graded set differs from rule to rule, and its size is recorded on every grade. A few weeks later, when the future has arrived, the library checks whether the highly ranked stocks actually beat the low ranked ones, and stores that as one grade per rule per day. The product is a decay chart per rule: is this rule still predicting, fading, or gone? *(The grades, the API route and the chart's specification exist; the chart itself is not yet drawn — its rendering is deferred to the UI workstream along with every other screen, so read the description below as what the read-model provides rather than as a screen you can open today.)*

**Why bother, when the arena already exists?** Three reasons. A rule's daily ranking produces hundreds of observations across the cross-section instead of one portfolio number, so a fading rule shows up in its grades years before a full trading operation could prove anything either way. That cross-sectional breadth is real, and it is worth being precise about what it does and does not buy: the verdict on whether a rule is fading rests on a five-year window of *overlapping* grades, which carries only **ten to twenty independent observations**, not hundreds. Both numbers are true and they answer different questions — the hundreds are what make each day's grade meaningful, the ten-to-twenty are what the trend claim actually stands on. That is why the effective sample is printed beside every flag, and why below a floor of ten the library declines to give a verdict at all rather than offering a confident-looking one: for the first couple of years of a rule's graded history there simply is not enough independent evidence to say, and saying so is the honest output. When a strategy loses, the grade separates "the rule stopped predicting" from "trading costs ate the edge," which the arena alone cannot tell apart. And the AI researcher's limited budget of experiments gets pointed at rule families whose grades say they still predict, instead of ones the whole market has already arbitraged away.
...
```mermaid
flowchart LR
    BARS["Daily prices +<br/>index membership"]:::machine --> SCORE["Each registered rule ranks<br/>the eligible stocks it can score"]:::machine
    ...
    HIST --> PANEL["Decay chart per rule<br/>stable / decaying / gone / not-yet<br/><i>(specified; not yet drawn)</i>"]:::outcome
...
Here is the split. The data foundation and the trading-and-accounting engine are built and tested, a strategy can trade a paper book against twenty years of validated market data with realistic costs, corporate actions, and recovery from outages. **The judging layer is built and tested too** — the fair-comparison scoring, the luck margin, the promotion gate, the overfitting monitor, the allocator and the random control crowd — and it has been exercised end to end by the sealed-room replay that calibrated it, over more than five thousand replayed trading days, which is what set its thresholds. The signal library is built but has not yet been run over history, so it holds no grades yet. What is specified in full detail but **not yet built** is the three AI seats; and no screen has been drawn yet, so the system is operated through its API rather than a dashboard. The AI trader and the researcher are designed down to the interface but are later phases of construction. So when this document describes the AI seats, it is describing *what the implementation will do when those phases are built*, from a complete specification, not something running today.
